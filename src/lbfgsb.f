...
C      if (iprint .ge. 1) then
C         write (6,1002) iter,f,sbgnrm
C         write (itfile,1003) iter,nfgv,sbgnrm,f
C      endif
...
c ----------------- the beginning of the loop --------------------------
 ...
 222  continue
C      if (iprint .ge. 99) write (6,1001) iter + 1
      iword = -1
c
...
c                            skip the L-BFGS update.
         nskip = nskip + 1
         updatd = .false.
C        if (iprint .ge. 1) write (6,1004) dr, ddum
         goto 888
      endif 
...
C        if (prjctd) write (6,*)
C    +   'The initial X is infeasible.  Restart with its projection.'
C        if (.not. cnstnd)
C    +      write (6,*) 'This problem is unconstrained.'
C     endif

C     if (iprint .gt. 0) write (6,1001) nbdd
...
C        if (iprint .ge. 0) write (6,*) 'Subgnorm = 0.  GCP = X.'
	 ...
	 return
      ...
      col2 = 2*col
      f1 = zero
C     if (iprint .ge. 99) write (6,3010)
...
C        if (iprint .gt. 100) write (6,1010) (xcp(i), i = 1, n)
         return
      endif    
...
c     Initialize derivative f2.
	 ...
	 if (info .ne. 0) return
      ...
      nint = 1
C     if (iprint .ge. 99) 
C    +   write (6,*) 'There are ',nbreak,'  breakpoints '
...
C     if (dt .ne. zero .and. iprint .ge. 100) then
C       write (6,4011) nint,f1,f2
C       write (6,5010) dt
c       write (6,6010) dtm
C     endif	     
...
C     if (iprint .ge. 100) write (6,*) 'Variable  ',ibp,'  is fixed.'
...
C        write (6,*)
C        write (6,*) 'GCP found in this segment'
C        write (6,4010) nint,f1,f2
C        write (6,6010) dtm
C     endif 
...
C     if (iprint .gt. 100) write (6,1010) (xcp(i),i = 1,n)
C     if (iprint .ge. 99) write (6,2010)
...
c                           count the entering and leaving variables.
	 do 20 i = 1, nfree
	    ...
	    if (iwhere(k) .gt. 0) then
	       ileave = ileave - 1
	       indx2(ileave) = k
C	       if (iprint .ge. 100) write (6,*)
C    +             'Variable ',k,' leaves the set of free variables'
            endif
  20     continue
	 do 22 i = 1 + nfree, n
	    k = index(i)
	    if (iwhere(k) .le. 0) then
	       ...
	       indx2(nenter) = k
C	       if (iprint .ge. 100) write (6,*)
C     +             'Variable ',k,' enters the set of free variables'
            endif
  22     continue
C         if (iprint .ge. 99) write (6,*)
C     +       n+1-ileave,' variables leave; ',nenter,' variables enter'
      endif
...
c     Find the index set of free and active variables at the GCP.
  ...
  24  continue
C      if (iprint .ge. 99) write (6,*)
C     +      nfree,' variables are free at GCP ',iter + 1  
...
C         write (6,*) 'N = ',n,'    M = ',m
C         if (iprint .ge. 1) then
C           write (itfile,2001) epsmch
C           write (itfile,*)'N = ',n,'    M = ',m
C	    write (itfile,9001)
C           if (iprint .gt. 100) then
C               write (6,1004) 'L =',(l(i),i = 1,n)
C               write (6,1004) 'X0 =',(x(i),i = 1,n)
C               write (6,1004) 'U =',(u(i),i = 1,n)
C            endif 
C         endif
C      endif 
...
C     if (iprint .ge. 99) then
C         write (6,*) 'LINE SEARCH',iback,' times; norm of step = ',xstep
C         write (6,2001) iter,f,sbgnrm
C         if (iprint .gt. 100) then	
C            write (6,1004) 'X =',(x(i), i = 1, n)
C            write (6,1004) 'G =',(g(i), i = 1, n)
C         endif
C      else if (iprint .gt. 0) then 
C         imod = mod(iter,iprint)
C         if (imod .eq. 0) write (6,2001) iter,f,sbgnrm
C     endif
C      if (iprint .ge. 1) write (itfile,3001)
C     +          iter,nfgv,nint,nact,word,iback,stp,xstep,sbgnrm,f
...
C         write (6,3003)
C         write (6,3004)
C         write(6,3005) n,iter,nfgv,nintol,nskip,nact,sbgnrm,f
C         if (iprint .ge. 100) then
C            write (6,1004) 'X =',(x(i),i = 1,n)
C         endif  
C         if (iprint .ge. 1) write (6,*) ' F =',f
C      endif 
 999  continue
C      if (iprint .ge. 0) then
C         write (6,3009) task
C         if (info .ne. 0) then
C            if (info .eq. -1) write (6,9011)
C           if (info .eq. -2) write (6,9012)
C            if (info .eq. -3) write (6,9013)
C            if (info .eq. -4) write (6,9014)
C            if (info .eq. -5) write (6,9015)
C            if (info .eq. -6) write (6,*)' Input nbd(',k,') is invalid.'
C            if (info .eq. -7) 
C    +	       write (6,*)' l(',k,') > u(',k,').  No feasible solution.'
C            if (info .eq. -8) write (6,9018)
C            if (info .eq. -9) write (6,9019)
C         endif
C         if (iprint .ge. 1) write (6,3007) cachyt,sbtime,lnscht
C         write (6,3008) time
C         if (iprint .ge. 1) then
C            if (info .eq. -4 .or. info .eq. -9) then
C               write (itfile,3002)
C     +             iter,nfgv,nint,nact,word,iback,stp,xstep
C            endif
C            write (itfile,3009) task
C            if (info .ne. 0) then
C               if (info .eq. -1) write (itfile,9011)
C               if (info .eq. -2) write (itfile,9012)
C               if (info .eq. -3) write (itfile,9013)
C               if (info .eq. -4) write (itfile,9014)
C               if (info .eq. -5) write (itfile,9015)
C               if (info .eq. -8) write (itfile,9018)
C               if (info .eq. -9) write (itfile,9019)
C            endif
C            write (itfile,3008) time
C         endif
C      endif
...
c     ************
      ...
      if (nsub .le. 0) return
C      if (iprint .ge. 99) write (6,1001)
...
C      if (iprint .ge. 99) then
C	 if (alpha .lt. one) then
C            write (6,1002) alpha
C         else
C            write (6,*) 'SM solution inside the box'
C	 end if	
C	 if (iprint .gt.100) write (6,1003) (x(i),i=1,n)
C      endif
...
C      if (iprint .ge. 99) write (6,1004)
